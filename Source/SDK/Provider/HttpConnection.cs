...
namespace Expressly.Api
{
    ...
    internal class HttpConnection
    {
        ...
        public string Execute(string payLoad, HttpWebRequest httpRequest)
        {
                        ...
                        using (WebResponse responseWeb = httpRequest.GetResponse())
                        {
                            ...
                            using (StreamReader readerStream = new StreamReader(responseWeb.GetResponseStream()))
                            {
                                ...
                                else
                                {
                                    ...
                                    logger.Debug("Code ["+ (int)this.ResponseDetails.StatusCode + " - " + this.ResponseDetails.StatusCode + "]. Body: " + this.ResponseDetails.Body);
                                }
            ...
            {
                // Rethrow any Expressly since they already contain the
            ...
            catch (System.Exception ex)
            {
                ...
                // the caller.
                throw new ExpresslyException("Exception in Expressly.HttpConnection.Execute(): " + ex.Message, ex);
            }

            // If we've gotten this far, it means all attempts at sending the
            // request resulted in a failed attempt.
            throw new ExpresslyException("Retried " + retriesConfigured + " times.... Exception in Expressly.HttpConnection.Execute(). Check log for more details.");
        }
    }
}
